## **1. Introduction**
Since the invention of Bitcoin in 2008, the crypto market has been introduced to more than 4000 cryptocurrencies. The market capitalization of Bitcoin [equals \$1 trillion in 2021](https://www.statista.com/statistics/377382/bitcoin-market-capitalization/#:~:text=The%20market%20capitalization%20of%20Bitcoin,circulation%20by%20the%20Bitcoin%20price), while the total market capitalization of the crypto overcame the [$1,7 trillion mark](https://coinmarketcap.com/charts/) and is expected to grow even further. However, the market is isolated from traditional finance and there is a lack of instruments tied to real profitability.

![](g1)

### **Market Problems**
As nearly all assets on the crypto market suffer from high volatility, it seems to be a logical starting point. During 2020 alone, the price of Bitcoin [has suffered](https://coinmarketcap.com/currencies/bitcoin/historical-data/) a 53% loss and then gone up 57,4%. Sure, this year has been rather unusual but the market situation was even more radical in 2019, let’s see the real price in USD now: the BTC price [has gone](https://coinmarketcap.com/currencies/bitcoin/historical-data/) up from \$5 167 on April 15 to \$10 907 on June 27 only to drop to \$7 980 on October 7. And the situation was similar in 2018 with a significant drop in price after Bitcoin boom in December 2017.

![](g2)

A solution to both volatility and lack of ties to real-world assets came in a form of stablecoins. Originally pegged to fiat money or exchange-traded commodities such as precious metals, stablecoins were presented as crypto-assets designed to not fluctuate in value — a safer option for investors looking to minimize their financial risks. In 2020, the market capitalization of stablecoins [overcame the $51 billion mark](https://www.coingecko.com/en/stablecoins).
...
***The secret of our cuisine is simple — we want to serve exquisite products to the crypto world by creating a perfect blend of traditional and crypto products.***
...
**Earn addition BAG token during USDp sell round.** 
Using this method, the community of the protocol will set an additional reward per 1 USDp token in BAG tokens to stimulate faster sale of new USDp tokens.
...
The main function of the intermediary is to find new borrowers, who are willing to lock their debt securities inorder to obtain liquidity from the protocol.
...
1. The borrower continues to own the securities (even though unable to sell them as they are locked into the collateral custody account) and receive interest income.
...
2. Distributed to those users who decided to lock their USDp/BAG into liquidity pools and provide the protocol with liquidity pool tokens (similar to SushiSwap)
3. Purchase at a special price in case of additional capitalization of the protocol for current BAG tokenholders (detailed explanation is available in section 3.3);
4. Each additional BAG tokens white USDp sale

To ensure the inexhaustibly of BAG — the protocol contains the ability to issue new tokens. The decision regarding the new issuance is taken is by voting of tokenholders.
...
After issuance, tokens are distributed to wallets or smart contracts. In order to ensure the moratorium on sale, the following restrictions will be set to wallets of founders **strictly during the moratorium period**:

1. The minimum balance of BAG tokens on the wallet (is equal to the initial investment amount);
2. Intransferability of BAG tokens from founders’ wallets.
...
While cooking the delicious technical stuffing of BondAppétit, we were inspired by a great work of developers from Compound, MakerDAO, Uniswap, and other DeFi protocols based on the Ethereum blockchain.
...
Another significant aim during the development of the protocol lied in ensuring the maximum security of BondAppétit. In order to achieve this, developers of the protocol conduct regular internal and external audits of smart contracts both during development and after their implementation on the main network.

We prepared a documentation for developers on our website - [http://bondappetit.io/docs](http://bondappetit.io/docs)
...
This smart contract acts as a depository that is used to balance the cost collateral and the volume of issued USDp.
...
The Market smart contract is used in order to sell USDp for a fixed price with an ability to receive rewards in a form of BAG tokens.

### **6.10 Depositary balance view**

The Depository Balance View is a smart contract that is used for calculation of the portfolio value in USD.  All Oracle contracts are obliged to implement this interface in order to communicate with the protocol.

### **6.11 Budget**

The Budget smart contract was designed for a simple but vital role - to distribute ETH for expenses of the protocol.

### **6.12 Investment**

The investment smart contract is used for the initial offering of BAG. Basically, this smart contact is entitled to sell BAG for a fixed price to investors.

### **6.13 Buyback**

The Buyback smart contact is a mean to buy BAG tokens from liquidity pools that include protocol's assets.
...
The open-source code tool that invokes smart contracts of the protocol. This utility by design can be deployed by any user of the network. In order to do this, a user will need to generate a new wallet in the ETH network and form an initial balance of 0.5 ETH. A fee for each transaction executed in the protocol will be automatically replenished by Budget.
...
During the second phase, the protocol will start the investment process. The goal of fundraising during this stage lies in purchasing RWA assets that will be collateralized and covering technical expenses of the protocol.

The investment process will be structured through an offering of BAG on the open market. The protocol aims to attract \$1.2M. \$1M will be used for the purchase of RWA and \$200 000 will cover the client’s expenses for fiat/crypto and crypto/fiat conversions and technical expenses or the protocol (oracles maintenance).

### Phase 3. RWA Collateralization

After raising funds, the protocol will initiate the next stage. After purchasing RWA, the protocol will collateralize these assets, which will strengthen USDp and will offer the first truly RWA-backed stablecoin on the market. Together with that, at this stage, the protocol will start receiving profits and distribute coupon payments from holding the RWA to BAG tokenholders.

### Phase 4. Direct Investment

The last stage of protocol’s launch. We expect that the protocol’s capitalization will reach \$100M at this point, which will make USDp exchange for other stablecoins and later exchange to fiat unnecessary. Together with that, during this stage, no more BAG tokens will be issued to the open market.
